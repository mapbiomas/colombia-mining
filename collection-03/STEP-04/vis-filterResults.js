...
var param = {
  regionCode:    30601,
  country:       'COLOMBIA',
  step:          'CF',   // 'CO': Original Classification | 'CF': Filtered Classification
  versionInput:  '11',    // Version of the output to load
...
years.forEach(function(year) {
  ...
  if (param.loadMining) {
    map.addLayer(
      classMining,
      { bands: ['classification_' + String(year)], min: 0, max: mapbiomasPalette.length-1, palette: mapbiomasPalette, format: 'png' },
      'Classification ' + year,
      true
    );
  }
...
var gridRows = [];
maps.forEach(function(_, i) {
  if (i % param.columnsPerRow !== 0) return;
